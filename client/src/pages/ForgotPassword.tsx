  ...
  return <div className="auth-shell">
    ...
    <main className="auth-main"><Card className="auth-card" pad>
      {step === 'email' ? <form className="stack" onSubmit={requestCode}>
        ...
        {error && <Alert tone="error">{error}</Alert>}
        <Field htmlFor="reset-code" label="Six-digit code"><Input id="reset-code" inputMode="numeric" maxLength={6} onChange={(e) => setCode(e.target.value.replace(/\D/g, '').slice(0, 6))} required value={code} /></Field>
